  ...
  const renderContent = useCallback(() => {
    ...
    return lines.map((line, lineIndex) => {
      ...
      if (headerMatch) {
        ...
        return (
          <HeaderTag key={lineIndex} className={`${className} cursor-text`} onClick={handleClick}>
      ...
      if (line.trim() === '') {
        return (
          <div key={lineIndex} className="mb-2 min-h-[1em] cursor-text" onClick={handleClick}>
      ...
      while (currentPos <= line.length) {
        ...
        if (isEditing && cursorPosition.start === lineStart + currentPos && !hasAddedCursor) {
        ...
        const blockAtPos = lineBlocks.find(block => 
          block.start === lineStart + currentPos
        );
        ...
        if (blockAtPos) {
          ...
          if (showRaw) {
            // Show raw text
            elements.push(
              <span key={`block-${blockAtPos.id}`} className="cursor-text" onClick={handleClick}>
                {blockAtPos.rawText}
              </span>
            );
          } else {
            ...
            const formattedElement = (
              <span key={`block-${blockAtPos.id}`} className="cursor-text" onClick={handleClick}>
                {blockAtPos.type === 'bold' && <strong>{blockAtPos.innerText}</strong>}
                {blockAtPos.type === 'italic' && <em>{blockAtPos.innerText}</em>}
          ...
          const nextBlock = lineBlocks.find(block => block.start > lineStart + currentPos);
          const endPos = nextBlock ? nextBlock.start - lineStart : line.length;
          
          if (endPos > currentPos) {
            const textSegment = line.substring(currentPos, endPos);
            elements.push(
              <span key={`text-${currentPos}`} className="cursor-text" onClick={handleClick}>
                {textSegment}
              </span>
            );
            currentPos = endPos;
