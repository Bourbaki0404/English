...
export function FlashCardQuizRoute() {
  ...
  const quiz =
    currentQuiz?.type === "flashcard"
      ? currentQuiz
      : getLatestQuizByTypeForCurrentDocument("flashcard");
  const cards = quiz ? (quiz.data as any[]) : [];

  return <FlashCardQuiz cards={cards} onBack={() => navigate("/")} />;
}
...
export function MultipleChoiceQuizRoute() {
  ...
  const quiz =
    currentQuiz?.type === "multiple-choice"
      ? currentQuiz
      : getLatestQuizByTypeForCurrentDocument("multiple-choice");
  const questions = quiz ? (quiz.data as any[]) : [];

  return (
    <MultipleChoiceQuiz questions={questions} onBack={() => navigate("/")} />
  );
}
...
export function ShortWritingQuizRoute() {
  ...
  const quiz =
    currentQuiz?.type === "short-writing"
      ? currentQuiz
      : getLatestQuizByTypeForCurrentDocument("short-writing");
  const tasks = quiz ? (quiz.data as any[]) : [];

  return <ShortWritingQuiz tasks={tasks} onBack={() => navigate("/")} />;
}
